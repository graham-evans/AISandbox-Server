...
public class PieChartTest {
    ...
    public void testNoData() throws IOException {
        PieChartWidget pieChart = PieChartWidget.builder().width(500).height(500).title("Empty Pie Chart").build();
        ImageIO.write(pieChart.getImage(), "png", new File(outputDir, "empty.png"));
    }

    @Test
    public void testOneData() throws IOException {
        PieChartWidget pieChart = PieChartWidget.builder().width(500).height(500).title("Pie Chart with 1 segment").build();
    ...
    public void testTwoData() throws IOException {
        PieChartWidget pieChart = PieChartWidget.builder().width(500).height(500).title("Pie Chart with 2 segments").build();
    ...
    public void testTenData() throws IOException {
        PieChartWidget pieChart = PieChartWidget.builder().width(500).height(500).title("Pie Chart with 10 segments").build();
        List<PieChartWidget.Slice> segments = new ArrayList<>();
        for (int i = 1; i <= 10; i++) {
            segments.add(new PieChartWidget.Slice("Agent " + i, (double) i, new Color(rand.nextFloat(), rand.nextFloat(), rand.nextFloat())));
        }
