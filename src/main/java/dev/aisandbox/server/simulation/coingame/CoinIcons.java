...
public class CoinIcons {
    ...
    public static BufferedImage[] getRowImages(int rowCount) {
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            drawCenteredString(g,"Row " + Integer.toString(i),new Rectangle(0,0,ROW_WIDTH,ROW_HEIGHT),new Font("Arial", Font.BOLD, 20));
        }
        return images;
    }
    ...
    public static BufferedImage[] getCoinImages(int cointCount) throws IOException {
        ...
        log.debug("loaded coins image of width {} and height {}", coinImage.getWidth(), coinImage.getHeight());
        // create the font
         ...
         //   g.fillRect(0, 0, COINS_WIDTH, ROW_HEIGHT);
            ...
            for (int j=0;j<crows.length;j++) {
                for (int k=0;k<crows[j];k++) {
                    g.drawImage(coinImage,(int)((k+j/2.0)*coinImage.getWidth())+xPadding,ROW_HEIGHT-coinImage.getHeight()-j*4-yPadding,null);
                }
            }
