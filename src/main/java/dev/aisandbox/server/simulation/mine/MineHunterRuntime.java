 ...
 * <p>
 * This class handles the execution of the Mine Hunter game, including board generation,
 * processing agent actions, updating game state, and visualizing the current state.
 * It implements the {@link Simulation} interface to integrate with the AI Sandbox framework.
 * </p>
 */
@Slf4j
public final class MineHunterRuntime implements Simulation {
   ...
   * This resets the game state and prepares for a new episode.
   */
  ...
  /**
   * Advances the simulation by one step, processing the agent's action
  ...
  /**
   * Creates a protocol buffer message representing the current state
   * of the game to be sent to the agent.
   *
  ...
  /**
   * Renders the current state of the simulation to a graphics context.
   * This includes the game board, statistics, and UI elements.
   *
  ...
  public void visualise(Graphics2D graphics2D) {
    ...
    GraphicsUtils.drawCenteredText(graphics2D, LEFT_MARGIN, TOP_MARGIN + TITLE_HEIGHT + WIDGET_SPACING + BAIZE_SPACING,
        BAIZE_WIDTH, OutputConstants.STATISTICS_HEIGHT, "Mines Remaining : " + board.getUnfoundMines(),
