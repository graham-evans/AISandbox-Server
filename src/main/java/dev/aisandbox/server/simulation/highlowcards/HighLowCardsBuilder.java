...
/**
 * Builder class for the High-Low Cards game simulation.
 * This class implements SimulationBuilder to provide the configuration
 * and instantiation of the HighLowCards simulation.
 * 
 * The High-Low Cards game presents a series of playing cards to the player,
 * who must predict whether the next card will be higher or lower than the current one.
 */
public final class HighLowCardsBuilder implements SimulationBuilder {
  ...
  /**
   * Specifies the minimum number of agents required for this simulation.
   * High-Low Cards is a single-player game, so only one agent is needed.
   * 
  ...
  /**
   * Specifies the maximum number of agents allowed for this simulation.
   * High-Low Cards is a single-player game, so only one agent is supported.
   * 
   ...
   * Currently, the High-Low Cards game doesn't have any configurable parameters.
   * 
  ...
  public Simulation build(List<Agent> agents, Theme theme, Random random) {
    // Create a new simulation with the first agent, 9 cards, the specified theme, and the random seed
