 ...
 * <p>
 * This interface defines the core contract for all AI Sandbox simulations. A simulation
 * represents a specific environment or game where AI agents can interact and be evaluated.
 * Each simulation is responsible for managing its own state, processing agent actions,
 * and providing visual output.
 * </p>
 * <p>
 * Simulations operate in a step-based manner where each call to {@link #step(OutputRenderer)}
 * advances the simulation by one logical unit (turn, time step, etc.). The simulation should
 * also be able to render its current state to a graphics context for visualization.
 * </p>
 * <p>
 * Example implementations include card games, maze navigation, multi-armed bandits, and
...
public interface Simulation {
   ...
   * <p>
   * This method is called when the simulation is ending, either because it has
   * completed naturally or because it has been manually stopped. Implementations
   * should clean up any resources such as network connections, file handles, or
   * background threads.
   * </p>
   * <p>
   ...
   * <p>
   * This method advances the simulation by one logical unit, which could be a turn,
   * a time step, or any other meaningful progression depending on the simulation type.
   * During this step, the simulation may:
   * </p>
   * <ul>
   ...
   * @throws SimulationException if an error occurs that requires the simulation to be stopped,
   *                             such as agent communication failures or invalid actions
   */
   ...
   * <p>
   * This method is called by an {@link OutputRenderer} to render the current state
   * of the simulation to a graphics context. The implementation should draw all
   * relevant visual elements including:
   * </p>
   * <ul>
