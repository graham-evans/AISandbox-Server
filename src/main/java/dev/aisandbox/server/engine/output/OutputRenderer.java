 ...
 * in a specific format or medium (e.g., visual display, file output, network stream, etc.). This
 * interface provides a standardized way to handle different types of output rendering while
 * maintaining flexibility for various implementation approaches.</p>
 *
...
public interface OutputRenderer {
   ...
   * with access to the simulation data it needs to perform its rendering tasks. Implementations
   * should use this opportunity to prepare any resources or initialize internal state based on the
   ...
   * frequency. For example, if framesToSkip is set to 2, the renderer will only render every second
   * frame (skipping 1, rendering 1).</p>
   *
   ...
   * sequences, video files, data exports). The provided directory will be used as the base location
   * for any files created by this renderer.</p>
   *
   ...
   *
   * @param outputDirectory the directory where output files should be created, may be null to use a
   *                        default location
   */
   ...
   * the rendered output with the current simulation state. Implementations should read the current
   * state from the simulation provided in {@link #setup(Simulation)} and render it according to
   * their specific output format.</p>
   *
   ...
   * cleanup of resources such as file handles, graphics contexts, network connections, or any other
   * system resources that the renderer may have acquired.</p>
   *
   * <p>The default implementation does nothing, indicating that this renderer
   * does not require explicit resource cleanup. Implementations that acquire resources should
   * override this method to ensure proper cleanup.</p>
   *
